 ...
 * 
 * Usage:
 *   tsx scripts/regenerate-books.ts [bookIds...]
 *   # Regenerate specific books:
 *   tsx scripts/regenerate-books.ts 1 2 3
 *   # Regenerate all books with multiple characters:
 *   tsx scripts/regenerate-books.ts
 */

import { getDb } from '../server/db';
import { books, bookCharacters, generationJobs } from '../drizzle/schema';
import { eq, inArray } from 'drizzle-orm';
import { claimAndRunJob } from '../server/generationWorker';

async function regenerateBooks(bookIds: string[]) {
